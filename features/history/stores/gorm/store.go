// Package r3hgorm provides a GORM-backed implementation of history.Store.
//
...
// It mirrors history.ChangeRecord but uses GORM-compatible types.
...
func (s *Store) GetVersion(
	...
	if result.Error != nil {
		...
		return history.ChangeRecord{}, fmt.Errorf("r3hgorm: get version: %w", result.Error)
	}
...
func (s *Store) LatestVersion(ctx context.Context, recordType string, recordID string) (history.ChangeRecord, error) {
	...
	if result.Error != nil {
		...
		return history.ChangeRecord{}, fmt.Errorf("r3hgorm: latest version: %w", result.Error)
	}
...
func (s *Store) queryRecords(query *gorm.DB, qarg ...r3.Query) ([]history.ChangeRecord, int64, error) {
	q := mergeQuery(qarg...)
	...
	records := make([]history.ChangeRecord, 0, len(models))
...
// modelToRecord converts a GORM model to an history.ChangeRecord.
func modelToRecord(m ChangeRecordModel) (history.ChangeRecord, error) {
	var record history.ChangeRecord

	record.ID = m.ID
	record.RecordType = m.RecordType
	record.RecordID = m.RecordID
	record.Action = history.Action(m.Action)
