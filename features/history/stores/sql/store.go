// Package r3hsql provides a database/sql-backed implementation of history.Store.
//
...
// Store is a database/sql-backed implementation of history.Store.
type Store struct {
	db   *sql.DB
	opts StoreOptions
}
...
func (s *Store) GetVersion(
	...
	if err != nil {
		...
		return history.ChangeRecord{}, fmt.Errorf("r3hsql: get version: %w", err)
	}
...
func (s *Store) LatestVersion(ctx context.Context, recordType string, recordID string) (history.ChangeRecord, error) {
	...
	if err != nil {
		...
		return history.ChangeRecord{}, fmt.Errorf("r3hsql: latest version: %w", err)
	}
...
func (s *Store) queryRecords(
	...
	var records []history.ChangeRecord
	for rows.Next() {
...
func (s *Store) scanRecord(row *sql.Row) (history.ChangeRecord, error) {
	var r history.ChangeRecord
	...
	r.Action = history.Action(action)
...
func (s *Store) scanRowRecord(rows *sql.Rows) (history.ChangeRecord, error) {
	var r history.ChangeRecord
	var changesRaw, metaRaw []byte
	var action string
