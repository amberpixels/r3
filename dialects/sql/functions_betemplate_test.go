...
func TestFilterToSQL_Template_SimpleComparison(t *testing.T) {
	...
	be.NoError(t, err)
	be.RequireThat(t, clauses, be.HaveLength(1))

	got := clauses[0].Clause
	be.AssertThat(t, got, be.Eq(`"id" = ?`))
	...
	be.AssertThat(t, got, be_string.MatchTemplate(
		`{{col}} {{op}} ?`,
...
func TestFilterToSQL_Template_InList(t *testing.T) {
	...
	got := clauses[0].Clause
	be.AssertThat(t, got, be.Eq(`"status" IN (?, ?, ?)`))
	...
	be.AssertThat(t, got, be_string.MatchWildcard(`"status" IN (?*?)`))
}
...
func TestFilterToSQL_Template_Between(t *testing.T) {
	...
	got := clauses[0].Clause
	be.AssertThat(t, got, be.Eq(`("age" >= ? AND "age" <= ?)`))
...
func TestFilterToSQL_Template_NestedAndOr(t *testing.T) {
	...
	got := clauses[0].Clause
	be.AssertThat(t, got, be.Eq(`("category" = ? AND ("price" < ? OR "on_sale" = ?))`))
	...
	be.AssertThat(t, got, be_string.MatchWildcard(`(* AND (* OR *))`))
	be.AssertThat(t, got, be_string.ContainingSubstring(" AND "))
	be.AssertThat(t, got, be_string.ContainingSubstring(" OR "))
}
...
func TestFilterToSQL_Template_IsNull(t *testing.T) {
	...
	got := clauses[0].Clause
	be.AssertThat(t, got, be.Eq(`"deleted_at" IS NULL`))
...
func TestFilterToSQL_Template_QualifiedColumnWithJoin(t *testing.T) {
	...
	got := clauses[0].Clause
	be.AssertThat(t, got, be.Eq(`"user"."name" = ?`))
	...
	be.AssertThat(t, clauses[0].Joins, be.Eq([]r3sql.SQLColumn{r3sql.SQLColumn(`"user"`)}))
}
