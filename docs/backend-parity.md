...
|---|---|---|---|---|---|
| Relationship filters - `r3.Has` (EXISTS) | ✅ | ❌ errors at SQL translation | ❌ | ✅ | ❌ ignored |
...
Nothing about the approach is backend-specific - GORM does it in
`drivers/gorm/relfilter.go`/`relaggregate.go` (shared `engine/sql.RelationMeta` +
`dialects/sql`); Mongo mirrors it in `engine/mongo/relfilter.go`/`relaggregate.go`
(pre-query `Distinct` on the related collection, then `$in`/`$nin` + a `$group`
pipeline). Still open: wiring the same lowering into `engine/sql.BaseCRUD` (raw SQL
drivers) and an in-memory form for file.
