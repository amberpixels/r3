...
func (r *BunCRUD[T, ID]) Create(ctx context.Context, entity T) (T, error) {
	...
	// A PK still zero means bun wrote no generated key back, so there is nothing
...
// write just touched.
func (r *BunCRUD[T, ID]) refreshPersisted(ctx context.Context, entity T) (T, error) {
	meta := enginesql.GetStructMeta[T]()

	fresh := new(T)
	read := r.db.NewSelect().Model(fresh).
		Where("? = ?", bun.Ident(meta.PKColumn), meta.PKValue(entity))
	if meta.SoftDeleteColumn != "" {
		read = read.WhereAllWithDeleted()
	}
	if err := read.Scan(ctx); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return entity, r3.ErrNotFound
		}
		return entity, err
