...
//   - Raw escape hatch (PgxRaw) for arbitrary SQL with $1-style placeholders
//
// Limitations / notes:
...
func NewPgxCRUD[T any, ID comparable](db *sql.DB) *PgxCRUD[T, ID] {
	...
	return &PgxCRUD[T, ID]{
		...
		meta: meta,
		raw:  NewPgxRaw[T, ID](db),
	}
}

// SetDefaultListQuery sets default ListQuery.
func (r *PgxCRUD[T, ID]) SetDefaultListQuery(q r3.Query) {
	r.defaultsMu.Lock()
...
func (r *PgxCRUD[T, ID]) SetDefaultGetQuery(q r3.Query) {
	r.defaultsMu.Lock()
	r.defaults.GetQuery = q
	r.defaultsMu.Unlock()
}

func (r *PgxCRUD[T, ID]) getDefaultListQuery() r3.Query {
	r.defaultsMu.RLock()
	defer r.defaultsMu.RUnlock()
	return r.defaults.ListQuery
}

func (r *PgxCRUD[T, ID]) getDefaultGetQuery() r3.Query {
	r.defaultsMu.RLock()
	defer r.defaultsMu.RUnlock()
	return r.defaults.GetQuery
}

func (r *PgxCRUD[T, ID]) Create(ctx context.Context, entity T) (T, error) {
...
func (r *PgxCRUD[T, ID]) List(ctx context.Context, qarg ...r3.Query) ([]T, int64, error) {
...
func (r *PgxCRUD[T, ID]) Get(ctx context.Context, id ID, qarg ...r3.Query) (T, error) {
	var entity T
...
func (r *PgxCRUD[T, ID]) Update(ctx context.Context, entity T) (T, error) {
	cols := r.meta.nonPKColumns()
	vals := r.meta.nonPKFieldValues(entity)
...
func (r *PgxCRUD[T, ID]) Delete(ctx context.Context, id ID) error {
	query := fmt.Sprintf(
...
// Raw returns the PgxRaw escape hatch for custom queries.
func (r *PgxCRUD[T, ID]) Raw() *PgxRaw[T, ID] { return r.raw }

// DB returns the underlying *sql.DB for advanced usage.
func (r *PgxCRUD[T, ID]) DB() *sql.DB { return r.db }
