// Package r3hmongo provides a MongoDB-backed implementation of r3history.Store.
//
...
// Store is a MongoDB-backed implementation of r3history.Store.
type Store struct {
...
func (s *Store) GetVersion(ctx context.Context, recordType string, recordID string, version int64) (r3history.ChangeRecord, error) {
	...
	if err != nil {
		...
		return r3history.ChangeRecord{}, fmt.Errorf("r3hmongo: get version: %w", err)
	}
...
func (s *Store) LatestVersion(ctx context.Context, recordType string, recordID string) (r3history.ChangeRecord, error) {
	...
	if err != nil {
		...
		return r3history.ChangeRecord{}, fmt.Errorf("r3hmongo: latest version: %w", err)
	}
...
func (s *Store) queryRecords(
	...
	records := make([]r3history.ChangeRecord, 0, len(docs))
...
// docToRecord converts a BSON document to an r3history.ChangeRecord.
func docToRecord(doc changeRecordDoc) (r3history.ChangeRecord, error) {
	var record r3history.ChangeRecord

	record.ID = doc.ID
	record.RecordType = doc.RecordType
	record.RecordID = doc.RecordID
	record.Action = r3history.Action(doc.Action)
