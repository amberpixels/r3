...
func DiffWithFields[T any](old, new T, fields []string) []FieldChange {
	if len(fields) == 0 {
		return nil
	}

	allChanges := Diff(old, new)
	if len(allChanges) == 0 {
		return nil
	}
...
func diffStructFields(oldVal, newVal reflect.Value, prefix string, changes *[]FieldChange) {
	...
	for i := range typ.NumField() {
		...
		if field.Type.Kind() == reflect.Pointer && field.Type.Elem().Kind() == reflect.Struct && field.Type.Elem() != timeType {
			switch {
