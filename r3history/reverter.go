...
func (r *Reverter[T, ID]) RevertTo(ctx context.Context, id ID, version int64) (T, error) {
	...
	if r.opts.DiffFunc != nil {
		...
		diffFn = func(old, new T) []FieldChange { return Diff(old, new) }
	}
	...
	if r.opts.MetadataFunc != nil {
		...
		record.Metadata.Extra = mergeExtra(record.Metadata.Extra, map[string]string{
			"reverted_to_version": fmt.Sprint(version),
		})
	} else {
	...
	if err := r.store.Record(ctx, record); err != nil {
		log.Printf("r3history: failed to record revert for %s/%s: %v", record.RecordType, record.RecordID, err)
	}

	// Evaluate snapshot rules after revert
	if len(r.opts.SnapshotRules) > 0 {
		meta := record.Metadata
		evaluateSnapshotRules(ctx, r.opts.SnapshotRules, record.RecordType, record.RecordID, record.Version, ActionRevert, current, result, meta)
	}
...
func (r *Reverter[T, ID]) RevertLast(ctx context.Context, id ID) (T, error) {
	...
	if latest.Version <= 1 {
		return zero, fmt.Errorf("r3history: cannot revert past the initial create (version 1)")
	}
