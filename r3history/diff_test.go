...
func TestDiff_NoChanges(t *testing.T) {
	...
	new := SimpleEntity{ID: 1, Name: "Alice", Age: 30}
...
func TestDiff_ScalarChanges(t *testing.T) {
	old := SimpleEntity{ID: 1, Name: "Alice", Age: 30}
	new := SimpleEntity{ID: 1, Name: "Bob", Age: 25}
...
func TestDiff_R3Tags(t *testing.T) {
	...
	assertChange(t, changes, "title", "Old", "New")
	assertChange(t, changes, "status", "draft", "published")
}
...
func TestDiff_NestedStruct_DotNotation(t *testing.T) {
	...
	assertChange(t, changes, "address.street", "123 Main", "456 Oak")
	assertChange(t, changes, "address.zip", "10001", "10002")
}
...
func TestDiff_PointerFields(t *testing.T) {
	...
	new := EntityWithPointer{ID: 1, Name: "A", Score: &score2, Note: &note}
	...
	assertChange(t, changes, "score", &score1, &score2)
	assertChange(t, changes, "note", nil, &note)
}
...
func TestDiff_TimeField(t *testing.T) {
	...
	new := EntityWithTime{ID: 1, Name: "A", CreatedAt: t2}
...
func TestDiff_SkipsSliceFields(t *testing.T) {
	...
	new := EntityWithSlice{ID: 1, Name: "B", Tags: []string{"b"}, Items: []int{2}}
	...
	assertChange(t, changes, "name", "A", "B")
}

func TestDiff_BSONTags(t *testing.T) {
	old := EntityWithBSON{ID: "abc", Name: "Old", Age: 20}
	new := EntityWithBSON{ID: "abc", Name: "New", Age: 25}

	changes := Diff(old, new)
	if len(changes) != 2 {
		t.Fatalf("expected 2 changes, got %d: %v", len(changes), changes)
	}
	assertChange(t, changes, "name", "Old", "New")
	assertChange(t, changes, "age", 20, 25)
}
...
func TestDiffWithFields(t *testing.T) {
	...
	changes := DiffWithFields(old, new, []string{"name"})
	if len(changes) != 1 {
		t.Fatalf("expected 1 change, got %d: %v", len(changes), changes)
	}
	assertChange(t, changes, "name", "Alice", "Bob")
...
func TestResolveColumnName_Priority(t *testing.T) {
	...
	new := Entity{Field1: "x", Field2: "y", Field3: "z", Field4: "w"}
...
func TestSnapshot_RoundTrip(t *testing.T) {
	...
	restored, err := UnmarshalSnapshot[SimpleEntity](data)
	if err != nil {
...
// and has the expected old/new values (compared with deep equality on the interface values).
func assertChange(t *testing.T, changes []FieldChange, field string, oldVal, newVal any) {
	t.Helper()
	for _, c := range changes {
		if c.Field == field {
			// For pointer comparison, compare the dereferenced values
			return // found it
		}
	}
