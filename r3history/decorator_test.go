...
func (s *memoryStore) Record(_ context.Context, record ChangeRecord) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *memoryStore) ForRecord(_ context.Context, recordType string, recordID string, _ ...r3.Query) ([]ChangeRecord, int64, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *memoryStore) ForType(_ context.Context, recordType string, _ ...r3.Query) ([]ChangeRecord, int64, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	var result []ChangeRecord
	for _, r := range s.records {
...
func (s *memoryStore) ForTree(_ context.Context, scopes []TreeScope, _ ...r3.Query) ([]ChangeRecord, int64, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	var result []ChangeRecord
	for _, r := range s.records {
...
func (s *memoryStore) GetVersion(_ context.Context, recordType string, recordID string, version int64) (ChangeRecord, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	for _, r := range s.records {
	...
	return ChangeRecord{}, ErrVersionNotFound
}

func (s *memoryStore) LatestVersion(_ context.Context, recordType string, recordID string) (ChangeRecord, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	var latest *ChangeRecord
	...
	if latest == nil {
		return ChangeRecord{}, ErrNoHistory
	}
	return *latest, nil
}
...
func (s *memorySnapshotStore) GetSnapshot(_ context.Context, recordType string, recordID string, version int64) (Snapshot, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *memorySnapshotStore) ListSnapshots(_ context.Context, recordType string, recordID string) ([]Snapshot, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	var result []Snapshot
	for _, snap := range s.snapshots {
...
func (s *memorySnapshotStore) LatestSnapshot(_ context.Context, recordType string, recordID string) (Snapshot, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	var latest *Snapshot
	...
	if latest == nil {
		return Snapshot{}, fmt.Errorf("no snapshots")
	}
	return *latest, nil
}
...
func TestHistoryCRUD_CreateRecordsHistory(t *testing.T) {
	crud := newMemoryCRUD()
	store := newMemoryStore()

	repo := WithHistory[Order, int64](crud, store,
		WithIDFunc[Order, int64](func(o Order) int64 { return o.ID }),
	)
	...
	records, count, err := store.ForRecord(ctx, "orders", fmt.Sprint(order.ID))
	if err != nil {
...
func TestHistoryCRUD_UpdateRecordsDiff(t *testing.T) {
	crud := newMemoryCRUD()
	store := newMemoryStore()
	...
	// Check history
	records, count, _ := store.ForRecord(ctx, "orders", fmt.Sprint(order.ID))
	if count != 2 {
		t.Fatalf("expected 2 records, got %d", count)
	}

	updateRec := records[1]
	if updateRec.Action != ActionUpdate {
...
func TestHistoryCRUD_PatchRecordsDiff(t *testing.T) {
	crud := newMemoryCRUD()
	store := newMemoryStore()
	...
	patchRec := records[1]
	if patchRec.Action != ActionPatch {
...
func TestHistoryCRUD_DeleteRecordsHistory(t *testing.T) {
	crud := newMemoryCRUD()
	store := newMemoryStore()
	...
	deleteRec := records[1]
	if deleteRec.Action != ActionDelete {
...
func TestHistoryCRUD_ReadsDontRecordHistory(t *testing.T) {
	crud := newMemoryCRUD()
	store := newMemoryStore()
...
func TestHistoryCRUD_MetadataFunc(t *testing.T) {
	crud := newMemoryCRUD()
	store := newMemoryStore()

	repo := WithHistory[Order, int64](crud, store,
		WithIDFunc[Order, int64](func(o Order) int64 { return o.ID }),
		WithMetadataFunc[Order, int64](func(_ context.Context) Metadata {
			return Metadata{
				ActorID:   "user_42",
				ActorType: "user",
				Source:    "api",
	...
	records, _, _ := store.ForRecord(ctx, "orders", fmt.Sprint(order.ID))
	if len(records) == 0 {
...
func TestHistoryCRUD_ChangesOnlyNoSnapshots(t *testing.T) {
	crud := newMemoryCRUD()
	store := newMemoryStore()
...
func TestHistoryCRUD_ParentRef(t *testing.T) {
	type Adset struct {
		ID         int64  `db:"id,pk" json:"id"`
	...
	repo := WithHistory[Adset, int64](crud, store,
		WithIDFunc[Adset, int64](func(a Adset) int64 { return a.ID }),
		WithRecordType[Adset, int64]("adsets"),
		WithParentRef[Adset, int64]("campaigns", "CampaignID"),
	)

	ctx := context.Background()
	adset, _ := repo.Create(ctx, Adset{CampaignID: 5, Name: "Test Adset"})

	records, _, _ := store.ForRecord(ctx, "adsets", fmt.Sprint(adset.ID))
	if len(records) == 0 {
		t.Fatal("expected at least 1 record")
	}
	...
	// Now test ForTree
	treeRecords, count, _ := store.ForTree(ctx, []TreeScope{
...
func TestReverter_RevertTo(t *testing.T) {
	...
	revertRec := records[2]
	if revertRec.Action != ActionRevert {
...
func TestHistoryCRUD_DeriveRecordType(t *testing.T) {
	crud := newMemoryCRUD()
	store := newMemoryStore()
...
func TestHistoryCRUD_SnapshotRules(t *testing.T) {
	crud := newMemoryCRUD()
	store := newMemoryStore()
	snapStore := newMemorySnapshotStore()

	// Snapshot rule: take a snapshot when status changes to "published"
	rule := SnapshotRule[Order]{
		Name:  "on_publish",
		Store: snapStore,
		Condition: func(action Action, old, new Order) bool {
			return old.Status != "published" && new.Status == "published"
		},
	}

	repo := WithHistory[Order, int64](crud, store,
		WithIDFunc[Order, int64](func(o Order) int64 { return o.ID }),
		WithSnapshotRules[Order, int64](rule),
	)
	...
	snaps, _ := snapStore.ListSnapshots(ctx, "orders", fmt.Sprint(order.ID))
	if len(snaps) != 0 {
	...
	restored, err := UnmarshalSnapshot[Order](snap.Data)
	if err != nil {
...
func TestHistoryCRUD_SnapshotRules_MultipleRules(t *testing.T) {
	crud := newMemoryCRUD()
	store := newMemoryStore()
	...
	rule1 := SnapshotRule[Order]{
		Name:  "on_publish",
		Store: snapStore1,
		Condition: func(_ Action, old, new Order) bool {
			return old.Status != "published" && new.Status == "published"
		},
	}

	// Rule 2: snapshot on high-value orders (total >= 1000)
	rule2 := SnapshotRule[Order]{
		Name:  "high_value",
		Store: snapStore2,
		Condition: func(action Action, _, new Order) bool {
			return action == ActionCreate && new.Total >= 1000
		},
	}

	repo := WithHistory[Order, int64](crud, store,
		WithIDFunc[Order, int64](func(o Order) int64 { return o.ID }),
		WithSnapshotRules[Order, int64](rule1, rule2),
	)
	...
	snaps1, _ := snapStore1.ListSnapshots(ctx, "orders", fmt.Sprint(order.ID))
	snaps2, _ := snapStore2.ListSnapshots(ctx, "orders", fmt.Sprint(order.ID))
