...
type DiffFunc[T any] func(old, new T) []FieldChange
...
// Options configures the behavior of a HistoryCRUD decorator.
...
// Option is a functional option for configuring HistoryCRUD.
...
func applyDefaults[T any, ID comparable](opts *Options[T, ID]) {
	...
	if opts.DiffFunc == nil {
		opts.DiffFunc = func(old, new T) []FieldChange {
			return Diff(old, new)
		}
	}
}
